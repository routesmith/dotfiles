...
- Use `prefix + y` to copy the newest tmux buffer to the system clipboard without pasting.
- Use `prefix + Y` to choose a tmux buffer in the native picker and copy it to the system clipboard; `prefix + =` keeps the default choose-and-paste behavior.
- Use `prefix + C-y` for an fzf buffer picker that previews full buffer contents and copies the selected buffer to the system clipboard.
