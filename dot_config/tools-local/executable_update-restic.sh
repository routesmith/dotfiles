...
PREV_VERSION_FILE="${BIN_DIR}/.restic.prev-version"
...
Usage:
  ...
  update-restic.sh --rollback     Roll back to the previously installed version
EOF
}
...
rollback() {
    ...
    echo "Rolling back restic to version $prev_version"

    install_ver "$prev_version"

    echo "Rollback complete: restic $($BIN version 2>/dev/null | awk 'NR == 1 {print $2}' || echo 'unknown')"
