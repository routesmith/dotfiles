...
PREV_REV_FILE="$TMUX_SRC/.prev-rev"
...
Usage:
  ...
  update-tmux.sh --rollback Roll back to the pre-update revision and rebuild
...
save_prev_state() {
    git -C "$TMUX_SRC" rev-parse HEAD > "$PREV_REV_FILE"
}
...
rollback() {
    ...
    echo "Rolling back tmux to revision $prev_rev"
    ...
    rolled_back_version="$(tmux_version_for_rev HEAD)"
    echo "Rebuilding tmux $rolled_back_version from revision ${prev_rev:0:12}..."
    ...
    echo "Rollback complete: tmux $($TMUX_PREFIX/bin/tmux -V)"
    echo "Rolled back to: ${prev_rev:0:12} ($rolled_back_version)"
}
