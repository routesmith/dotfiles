...
Usage:
  ...
  update-chezmoi.sh --rollback Print rollback guidance (chezmoi manages its own binary)
...
rollback_guidance() {
  ...
  chezmoi manages its own binary lifecycle; automated rollback is not supported.

  To revert to the previous chezmoi version, choose one of these options:

  1. Use chezmoi's built-in upgrade/downgrade:
     chezmoi upgrade
     # or to pin a specific version:
     chezmoi upgrade --version=<previous-version>

  2. Rebuild from the local source checkout:
     cd $CHEZMOI_SRC
  ...
  3. Re-apply from a known-good chezmoi source version:
     cd $CHEZMOI_SRC
     git checkout <tag>            # e.g. v2.52.0
     make build-in-git-working-copy
     install -m 0755 chezmoi $INSTALL_TARGET
